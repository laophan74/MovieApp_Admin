...
namespace MovieApp_Admin
{
    public partial class Actor : Form
    {
        ...
        private void guna2Button5_Click(object sender, EventArgs e)
        {
            ...
            {
                DocumentReference docref = db.Collection("Actors").Document(guna2DataGridView1.CurrentRow.Cells[2].Value.ToString());
                docref.DeleteAsync();
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                guna2DataGridView1.Rows.Clear();
                searchQ = db.Collection("Actors").WhereGreaterThanOrEqualTo("name", searchstr)
                        .WhereLessThanOrEqualTo("name", searchstr + "\uf8ff");

                QuerySnapshot searchSS = await searchQ.GetSnapshotAsync();
                foreach (var item in searchSS)
                {
                    DocumentReference docRef = db.Collection("Actors").Document(item.Id);
                    DocumentSnapshot snapshot = await docRef.GetSnapshotAsync();
                    InfoActor d = snapshot.ConvertTo<InfoActor>();
                    Image image = myRes._default;
                    if (snapshot.Exists)
                    {
                        if (d.avatar != "")
